...
function renderImportedCompanyCard(chosen){if(!chosen?.Name||!document.body)return;const id='tallysync-imported-company-card';let old=document.getElementById(id);if(old)old.remove();const allText=[...document.querySelectorAll('body *')].filter(e=>e.children.length===0&&/Select Company/i.test(e.textContent||''));const anchor=allText[0];const card=document.createElement('div');card.id=id;card.setAttribute('data-tallysync-imported-company','1');card.style.cssText='margin-top:10px;padding:14px 16px;border:1px solid #cbd5e1;border-radius:12px;background:#fff;box-shadow:0 3px 12px #00000012;cursor:pointer;font:inherit;';card.innerHTML=`<div style="font-weight:800;color:#0f172a">${esc(chosen.Name)}</div><div style="font-size:12px;color:#64748b;margin-top:4px">${esc(chosen.GSTIN||'GSTIN not available')}${chosen.StateName?' • '+esc(chosen.StateName):''}</div><div style="font-size:11px;color:#2563eb;margin-top:7px">✓ Imported from Tally • Click to make Active Company</div>`;card.onclick=()=>{localStorage.setItem(K.company,JSON.stringify(chosen));emit(K.companyData,chosen);window.TallySyncActiveCompany=chosen;document.querySelectorAll('[data-active-company]').forEach(el=>el.textContent=chosen.Name);window.dispatchEvent(new CustomEvent('tallysync:company-updated',{detail:{companies:JSON.parse(localStorage.getItem(K.companies)||'[]'),selected:chosen}}));card.style.borderColor='#2563eb';card.style.background='#eff6ff';};if(anchor){let p=anchor;for(let i=0;i<6&&p.parentElement;i++){if(p.parentElement.children.length<=12)p=p.parentElement;else break}p.insertAdjacentElement('afterend',card)}else{card.style.position='fixed';card.style.top='120px';card.style.right='24px';card.style.zIndex='99998';document.body.appendChild(card)}}
function syncCompanyUI(rows,chosen){const all=[...new Map(rows.filter(x=>x?.Name).map(x=>[x.Name.toLowerCase(),x])).values()];const selectors=['#companySelect','#company-select','#companyDropdown','#company-dropdown','select[name="company"]','select[name="companySelect"]','[data-company-select]','[data-company-list]','[data-companies]'];selectors.forEach(sel=>document.querySelectorAll(sel).forEach(el=>{if(el.tagName==='SELECT'){const old=el.value;el.innerHTML='';all.forEach(x=>{const o=document.createElement('option');o.value=x.Name;o.textContent=x.Name+(x.GSTIN?' — '+x.GSTIN:'');if((chosen&&x.Name===chosen.Name)||x.Name===old)o.selected=true;el.appendChild(o)});el.dispatchEvent(new Event('change',{bubbles:true}))}else{el.textContent=chosen?.Name||''}}));document.querySelectorAll('[data-active-company]').forEach(el=>el.textContent=chosen?.Name||'');const detail={companies:all,selected:chosen};['tallysync:company-updated','tallysync:companies','tallysync:company-imported'].forEach(n=>window.dispatchEvent(new CustomEvent(n,{detail}));try{window.postMessage({type:'TALLYSYNC_COMPANIES_IMPORTED',companies:all,selected:chosen},'*')}catch{}window.TallySyncCompanies=all;window.TallySyncActiveCompany=chosen;renderImportedCompanyCard(chosen)}
