/* Remote Tally connector UI v10 - robust company connection test */
(()=>{
    ...
    btn.onclick=async()=>{
      ...
      try{
        ...
        for(const n of nodes){
          const a=n.getAttribute&& (n.getAttribute('NAME')||n.getAttribute('Name'));
          const t=(n.textContent||'').trim();
          if(/^COMPANY$/i.test(n.tagName)&&a) {name=a;break;}
          if(/^NAME$/i.test(n.tagName)&&t) {name=t;break;}
        ...
        if(companyNode){
          for(const k of ['Guid','MailingName','StateName','PinCode','PhoneNumber','Email','GSTIN']){
            const el=[...companyNode.getElementsByTagName('*')].find(n=>n.tagName.toLowerCase()===k.toLowerCase());
            company[k]=(el?.textContent||companyNode.getAttribute(k)||companyNode.getAttribute(k.toUpperCase())||'').trim();
          }
        }
        localStorage.setItem('tallysync_selected_company',JSON.stringify(company));
        localStorage.setItem('tallysync_company_profile',JSON.stringify(company));
