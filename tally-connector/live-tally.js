(()=>{
  ...
  function panel(){
    ...
    root.innerHTML=`<div style="background:#fff;border:1px solid #dbe2ea;border-radius:18px;box-shadow:0 8px 30px #0f172a14;overflow:hidden"><div style="padding:18px 20px;background:linear-gradient(135deg,#0f172a,#1e3a8a);color:#fff;display:flex;justify-content:space-between;gap:12px;align-items:center;flex-wrap:wrap"><div><div style="font-size:22px;font-weight:800">ANISH TECHNOLOGIES · Live Tally</div><div style="font-size:12px;opacity:.78;margin-top:4px">Real-time company, ledger, stock and voucher data</div></div><div id="ts-live-state" style="font-size:12px">Disconnected</div></div><div style="padding:16px"><div style="display:flex;gap:8px;flex-wrap:wrap"><input id="ts-live-url" value="${esc(saved())}" placeholder="Direct connector: http://192.168.x.x:9101" style="flex:1;min-width:260px;padding:10px;border:1px solid #cbd5e1;border-radius:9px"><input id="ts-live-code" value="${esc(code())}" placeholder="Remote Office Code" style="width:180px;padding:10px;border:1px solid #cbd5e1;border-radius:9px"><button id="ts-live-save" style="padding:10px 14px;border:0;border-radius:9px;background:#2563eb;color:#fff">Save</button><button id="ts-live-test" style="padding:10px 14px;border:0;border-radius:9px">Test Tally</button></div><div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-top:10px"><span style="font-size:12px;font-weight:700">Selected Company:</span><select id="ts-live-company" style="min-width:280px;padding:9px;border:1px solid #cbd5e1;border-radius:9px"><option value="">— Fetch Companies —</option></select><button id="ts-live-fetch" style="padding:9px 12px;border:0;border-radius:9px;background:#0f766e;color:#fff">Fetch Companies</button></div><div id="ts-live-msg" style="font-size:12px;margin:9px 0;color:#475569"></div><div style="display:flex;gap:7px;flex-wrap:wrap;margin:12px 0"><button data-tab="companies">Companies</button><button data-tab="ledgers">Ledgers</button><button data-tab="stock">Stock Items</button><button data-tab="sales">Sales Invoices</button><button data-tab="purchases">Purchase Vouchers</button></div><div id="ts-live-content" style="min-height:90px"></div></div></div>`;
    document.body.prepend(root);
    const msg=root.querySelector('#ts-live-msg'),state=root.querySelector('#ts-live-state'),content=root.querySelector('#ts-live-content'),companySel=root.querySelector('#ts-live-company');
    const buttons=[...root.querySelectorAll('[data-tab]')];
    buttons.forEach(b=>b.style.cssText='padding:8px 12px;border:1px solid #cbd5e1;border-radius:8px;background:#f8fafc;cursor:pointer');
    ...
    async function fetchCompanies(){
      msg.textContent='⏳ Fetching companies from Tally...';
      try{const raw=await collection('Company Collection','Company','Name,Guid,MailingName,StateName,PinCode,PhoneNumber,Email,GSTIN');const rows=parseRows(raw,['Name','Guid','MailingName','StateName','PinCode','PhoneNumber','Email','GSTIN']);
        companySel.innerHTML='<option value="">— Select Tally Company —</option>'+rows.map((c,i)=>`<option value="${i}">${esc(c.Name)}</option>`).join('');companySel._rows=rows;msg.textContent=`✅ ${rows.length} company record(s) found`;
      }catch(e){msg.textContent='❌ '+e.message;companySel.innerHTML='<option value="">— Fetch failed —</option>'}
    }
    root.querySelector('#ts-live-fetch').onclick=fetchCompanies;
    companySel.onchange=()=>{const c=companySel._rows?.[Number(companySel.value)];if(!c)return;const obj={name:c.Name,guid:c.Guid,gstin:c.GSTIN,state:c.StateName,pin:c.PinCode,phone:c.PhoneNumber,email:c.Email,address:c.MailingName};localStorage.setItem(K.company,JSON.stringify(obj));msg.textContent='✅ Selected company: '+c.Name;};
    try{const c=selected();if(c)msg.textContent='Selected: '+c.name}catch{}
    ...
    async function load(tab){
      ...
      try{let raw,rows,cols;
        if(tab==='companies'){raw=await collection('Company Collection','Company','Name,Guid,MailingName,StateName,PinCode,PhoneNumber,Email,GSTIN');rows=parseRows(raw,['Name','Guid','MailingName','StateName','PinCode','PhoneNumber','Email','GSTIN']);cols=[['Name','Company'],['GSTIN','GSTIN'],['StateName','State'],['PinCode','PIN'],['PhoneNumber','Phone'],['Email','Email']];}
        else if(tab==='ledgers'){raw=await collection('Ledger Collection','Ledger','Name,Parent,OpeningBalance,ClosingBalance,Address,StateName,GSTIN,PhoneNumber,Email');rows=parseRows(raw,['Name','Parent','OpeningBalance','ClosingBalance','StateName','GSTIN','PhoneNumber','Email']);cols=[['Name','Ledger'],['Parent','Group'],['OpeningBalance','Opening'],['ClosingBalance','Closing'],['StateName','State'],['GSTIN','GSTIN'],['PhoneNumber','Phone']];}
        else if(tab==='stock'){raw=await collection('Stock Item Collection','Stock Item','Name,Parent,ClosingBalance,ClosingValue,Rate,BaseUnits,PartNo,HSNCode');rows=parseRows(raw,['Name','Parent','ClosingBalance','ClosingValue','Rate','BaseUnits','PartNo','HSNCode']);cols=[['Name','Stock Item'],['Parent','Group'],['ClosingBalance','Closing Qty'],['ClosingValue','Value'],['Rate','Rate'],['BaseUnits','Unit'],['HSNCode','HSN']];}
        else {const type=tab==='sales'?'Sales':'Purchase';raw=await collection(`${type} Voucher Collection`,'Voucher','Date,VoucherNumber,VoucherTypeName,PartyLedgerName,Amount,Narration,MasterID').catch(()=>collection('Day Book','Voucher','Date,VoucherNumber,VoucherTypeName,PartyLedgerName,Amount,Narration,MasterID'));rows=parseRows(raw,['Date','VoucherNumber','VoucherTypeName','PartyLedgerName','Amount','Narration','MasterID']).filter(r=>!r.VoucherTypeName||r.VoucherTypeName.toLowerCase().includes(type.toLowerCase()));cols=[['Date','Date'],['VoucherNumber','Voucher No.'],['VoucherTypeName','Type'],['PartyLedgerName','Party'],['Amount','Amount'],['Narration','Narration']];}
